...
(function (root) {
  ...
  function bookEquipment() {
    ...
    out.innerHTML =
      ...
      '<div class="callout info"><strong>Booking links ready.</strong> This static site can\u2019t write to Google directly, so each link opens the event <em>pre-filled on that equipment\u2019s own calendar</em> \u2014 just click <em>Save</em> in Google Calendar (you must have edit access to the calendar).</div>' +
      '<ol class="book-links">' + links + '</ol>' +
      '<p><a class="btn ghost" href="' + icsUrl + '" download="equipment-bookings.ics">Download all as .ics</a> ' +
      '<button type="button" class="btn ghost" id="openAllBtn">Open all links</button></p>';
    var openAll = document.getElementById('openAllBtn');
    if (openAll) openAll.onclick = function () { bookings.forEach(function (b) { window.open(gcalLink(b.cal, b.title, b.date, b.start, b.end), '_blank', 'noopener'); }); };
  }
