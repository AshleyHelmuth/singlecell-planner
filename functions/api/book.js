 ...
 *
 * One-click equipment booking: writes an event to the correct lab equipment
 * Google Calendar using a SERVICE ACCOUNT (single shared identity).
 *
 * ── Setup you must do (once) ──────────────────────────────────────────────
 *  1. Google Cloud: create a project, enable the "Google Calendar API".
 *  2. Create a Service Account; create a JSON key for it; download the JSON.
 *  3. Share EACH equipment calendar with the service account's email
 *     (calendar_id ... Settings ... "Share with specific people" ...
 *      add the client_email ... permission "Make changes to events").
 *  4. In Cloudflare Pages ... your project ... Settings ... Variables and Secrets,
 *     add a SECRET named  GOOGLE_SA_KEY  whose value is the ENTIRE contents of
 *     the downloaded JSON key file (paste it verbatim). Mark it encrypted.
 *  5. Redeploy. Done. Nothing secret ever lives in the repo.
 *
 * The service-account key is read only at runtime from env.GOOGLE_SA_KEY and is
 * never sent to the browser. Only the allow-listed calendars below can be
 * written to, so a stray request can't touch arbitrary calendars.
 * ──────────────────────────────────────────────────────────────────────────
 */

// Allow-list: equipment name -> its Google calendar id. Must match scheduling.js.
// (Calendar ids are not secret; they already ship in the client. The point of
//  keeping the map here too is to refuse writes to anything not on this list.)
const CALENDARS = {
...
export async function onRequestPost(context) {
  ...
  try {
    if (!env.GOOGLE_SA_KEY) {
      return json({ error: 'not_configured', message: 'GOOGLE_SA_KEY secret is not set in Cloudflare.' }, 503);
    }
    let body;
    ...
    return json({ error: 'exception', message: (e && e.message) ? e.message : String(e) }, 500);
  }
}

// Friendly GET so you can sanity-check the endpoint exists / is configured.
export async function onRequestGet(context) {
  return json({
    ok: true,
    endpoint: '/api/book',
    configured: !!context.env.GOOGLE_SA_KEY,
